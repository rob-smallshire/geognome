    ...
    def Solve(self, maxGenerations):
        ...
        for generation in range(maxGenerations):
            ...
            for candidate in range(self.nPop):
                ...
                # Bring the trial solution into range
                for j in range(0, len(self.trialSolution)):
                    if self.trialSolution[j] < self.min[j]:
                        self.trialSolution[j] = self.min[j]
                    if self.trialSolution[j] > self.max[j]:
                        self.trialSolution[j] = self.max[j]
            ...
            for i in range(len(self.bestSolution)):
                print "[" + str(i) + "]: " + str(self.bestSolution[i])
            print "with energy %f" % self.bestEnergy
